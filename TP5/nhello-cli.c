...
int dialoguer_avec_serveur_nHello(int soc,struct sockaddr_un *adr_serveur){
	struct sockaddr_un adr_client;
	char buf1[2048],buf2[2048];
	//on fabrique la requete
	sprintf(buf2,"HELLO");
	...
	for (int i = 0; i < 10000; ++i){
		...
		printf("Envoi de la requete %d/10000 fois...\n",i+1);
	}
	sprintf(buf2,"NUMBER");
	int k=bor_sendto_un_str(soc,buf2,adr_serveur);
	if(k<0){
...
int main (int argc,char * argv[]){
	...
	while(boucle_prime){
		k=dialoguer_avec_serveur_nHello(soc,&adr_serveur);
		if(k<0){
			break;
		}
	}
